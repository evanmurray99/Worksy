			...
			if (response.status === 201) {
				...
				return Promise.all(categoryAddRequests).then((categoryAddResults) => {
					if (categoryAddResults.some((result) => !result)) {
						...
						return rollback(serviceId, categories).then((rollbackResult) => {
							return {
								success: false,
								error: 'Category addition failed',
						...
						return axios(userServicesUrl, userServicesRequest)
							.then((userServicesResponse) => {
								if (userServicesResponse.status === 200) {
									return {
										success: true,
										message: 'Service created and added to categories and user',
									};
								} else {
									return {
										success: false,
										error: `Could not update user's services. Error: ${userServicesResponse.status}`,
									};
								}
							})
							.catch((userServicesError) => {
								return {
									success: false,
									error: `Could not update user's services: ${
										userServicesError.response
											? userServicesError.response.data.message
				...
				return {
					success: false,
					error: `Could not create service. Error: ${response.status}`,
				};
			}
		})
		.catch((error) => {
...
const rollback = (serviceId, categories) => {
	...
	return Promise.all(categoryRemoveRequests).then((categoryRemoveResults) => {
	...
	return axios
		...
		.then((response) => {
			if (response.status === 200) {
				...
				return {
					success: false,
					error: `Failed to edit Service. Error: ${response.status}`,
				};
			}
		})
		.catch((error) => {
...
export { editService, deleteService, addService };
