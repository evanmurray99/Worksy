...
const sendMessage = () =>{
    if(selected!=='' && message.trim() !== ''){
        const url = `http://localhost:3001/api/chats/${selected}`;
        fetch(url, {
			method: 'PUT',
            headers: {
			...
			.then((data) => {
				socket.emit('messageSent', { chat: getSelectedChat(), senderId: loggedInUser._id, message });
                setTrigger((prevTrigger)=>!prevTrigger)
			})
                    ...
                    <div className='selectedChat'>
                            ...
                            <div className = 'messageBox'>
                                <div className='messageList'>
                                    <MessageList loggedInUser={loggedInUser} chat={getSelectedChat()}  trigger={trigger}/>
                                </div>
