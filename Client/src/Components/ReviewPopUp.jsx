...
export default function ReviewPopUp({
          ...
          setUser(
            await Promise.all(
              reviews.map(async (rev) => {
                const yyresponse = await fetch(
                  `http://localhost:3001/api/users/${rev.reviewer}`
                );
  ...
  const handleSubmitedReview = async (event) => {
    ...
    const firstName = userReviewer.map(
      (innerArray) => innerArray.reviewObject.firstName
    );
      ...
      {user ? (
        ...
        >
              ...
              {userReviewer.map((userReviewer, index) => (
                <div key={index} className="review-item">
                  <p className="review-text">
                    {userReviewer.reviewDataKey.text}
                  </p>

                  <div className="review-star-container">
                    <p className="review-user">
                      By: {userReviewer.reviewObject.firstName}
                    </p>
                  </div>
                    ...
                    {[1, 2, 3, 4, 5].map((star) => (
                      <span
                        ...
                        style={{
                          fontSize: "25px",
                          color:
                            star <= userReviewer.reviewDataKey.rating
                              ? "gold"
                              : "gray",
                        }}
                      >
          ...
          {showForm === true ? (
            <div className="review-form">
              ...
              <textarea
                ...
                rows="4"
                value={comment}
                placeholder="Write your reivew here..."
          ...
          <div className="buttons-container">
            <button className={buttonClasses} onClick={() => closePopUp(false)}>
              Close
            </button>
            {showForm === true ? (
              <button className="submit-button" onClick={handleSubmitedReview}>
                Submit
              </button>
            ) : (
              <></>
